...
def build_accumulator(
    ...
    Parameters:
        - ground_truth: DetectionOutput or TrackingOutput from annotation files.
        - predictions: DetectionOutput or TrackingOutput from a detector or tracker.
        - iou_threshold: minimum IoU for a match; pairs below this are excluded.
    Returns:
        Populated MOTAccumulator ready for metric computation.
    """
    # Auto-detect use_identity from input types: True for TrackingOutput, False for DetectionOutput
    use_identity = isinstance(ground_truth, TrackingOutput) and isinstance(predictions, TrackingOutput)
    ...
    for frame_index, gt_detections, pred_detections in _iter_frame_pairs(ground_truth, pred_index, frame_stride):
        if use_identity:
            ...
            # Detection-only evaluation ignores identity and uses per-frame placeholder IDs.
            gt_ids   = list(range(len(gt_detections)))
            pred_ids = list(range(len(pred_detections)))
...
def build_hota_data(
    ...
    for gt_pos, frame in enumerate(ground_truth.frames):
        for detection in frame.detections:
            ...
            if detection.class_name not in gt_class_to_id:
                gt_class_to_id[detection.class_name] = len(gt_class_to_id)
        for detection in pred_index.get(gt_pos * frame_stride, []):
