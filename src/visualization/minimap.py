...
# Layout: width-fit a 28 m court onto a 1120 px canvas.
# _MARGIN controls the pixel border around the court; pass margin=0 to callers
# that want edge-to-edge rendering (overlays), or the default 60 px for
# standalone videos that need visual breathing room.
_CANVAS_W = 1120
_MARGIN = 60
_PADDING_MM = 1000.0
...
def _polyline_to_px(pts_mm: np.ndarray, margin: int = _MARGIN) -> np.ndarray:
    return np.array(
        [world_to_px(float(p[0]), float(p[1]), margin) for p in pts_mm],
        dtype=np.int32,
    )


def draw_court(canvas: np.ndarray, margin: int = _MARGIN) -> None:
    """Stroke court lines and hoop centers onto an existing canvas in place."""
    for line in cp.court_lines():
        cv2.polylines(
            canvas, [_polyline_to_px(line.pts, margin)], line.closed, _COURT_COLOR, 1, cv2.LINE_AA
        )
    for x_mm in (cp.LEFT_RIM_X, cp.RIGHT_RIM_X):
        cv2.circle(canvas, world_to_px(x_mm, 0.0, margin), 4, _HOOP_COLOR, -1, cv2.LINE_AA)


def make_base_canvas(margin: int = _MARGIN) -> np.ndarray:
    """Allocate a fresh canvas and stroke the court lines on it."""
    _, canvas_h = canvas_size(margin)
    canvas = np.full((canvas_h, _CANVAS_W, 3), _BG_COLOR, dtype=np.uint8)
    draw_court(canvas, margin)
    return canvas
...
def draw_landmark_dots(
    ...
    for x_mm, y_mm, color, label in dots:
        px, py = world_to_px(x_mm, y_mm, margin)
...
def draw_dot(canvas: np.ndarray, point: Point3D, margin: int = _MARGIN) -> None:
    ...
    """
    px, py = world_to_px(float(point.x), float(point.y), margin)
    if point.is_ball:
